rosu-pp-ffi is intended to provide C-FFI to the Rust library rosu-pp. Other relevant libraries are rosu-map and rosu-mods. The source code of all dependencies is available in the locale registry at `~/.cargo/registry/`.

The header file `/include/rosu_pp.h` is generated automatically via cbindgen; do not modify it manually. cbindgen is ran via `build.rs`.

Documentation should be provided to items of the public interface. Generally, the corresponding rosu-pp documentation can be used. Occasionally, additional documentation information should be included for pointer management. E.g. when a *mut is taken as argument and converted to a Box which is then dropped, in which case users should no longer try to access the pointer.

## Code style

Some additional formatting rules:
- Always add a blank line before return statements, and lines containing key words like `continue` or `break`.
- Always add blank lines before and after multi-line statements.
